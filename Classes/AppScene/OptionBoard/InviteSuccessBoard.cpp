...
bool InviteSuccessBoard::init()
{
	if (!OptionBaseBoard::init(kInviteSuccessBoard))
	{
		return false;
	}
	initRootCSBNode();
...
void InviteSuccessBoard::initRootCSBNode()
{
...
void InviteSuccessBoard::handleButton()
{
	//button
	CSBHelper::registerButton(m_rootCSBNode, "ButtonClose", CC_CALLBACK_2(InviteSuccessBoard::btnCloseCallBack, this));

}

void InviteSuccessBoard::updateLanguage()
{
	//ok menu
	...
	// info
	string infoStr = STLang::shared()->getValue("Congratulation! Check new Backdrop in option");
...
void InviteSuccessBoard::closeEffect()
{
	...
	ST_RETURN_IF(!ease);
	CallFunc *callFunc = CallFunc::create(this, callfunc_selector(InviteSuccessBoard::removeSelfCallFunc));
...
void InviteSuccessBoard::btnCloseCallBack(Ref *pSender, Widget::TouchEventType type)
{
	switch (type)
	{
