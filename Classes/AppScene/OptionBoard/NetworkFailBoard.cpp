...
bool NetworkFailBoard::init()
{
	if (!OptionBaseBoard::init(kNetworkFailBoard))
	{
		return false;
	}
	initRootCSBNode();
...
void NetworkFailBoard::initRootCSBNode()
{
...
void NetworkFailBoard::handleButton()
{
	//button
	CSBHelper::registerButton(m_rootCSBNode, "ButtonClose", CC_CALLBACK_2(NetworkFailBoard::btnCloseCallBack, this));

}

void NetworkFailBoard::updateLanguage()
{
	//ok menu
	...
	// info
	string infoStr = STLang::shared()->getValue("NetworkFailInfo");
	Size bgSize = getBgSize();
	float maxWidth = bgSize.width*0.85;
	CSBHelper::updateFntTextToLabel(m_rootCSBNode, "TextInfo", infoStr, maxWidth, TextHAlignment::LEFT);
}


void NetworkFailBoard::closeEffect()
{
	...
	ST_RETURN_IF(!ease);
	CallFunc *callFunc = CallFunc::create(this, callfunc_selector(NetworkFailBoard::removeSelfCallFunc));
...
void NetworkFailBoard::btnCloseCallBack(Ref *pSender, Widget::TouchEventType type)
{
	switch (type)
	{
